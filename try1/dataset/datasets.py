...
class PFCNDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(128,128), mean=(128,128,128), scale=True, mirror=True):
        self.root = root
        self.list_path = list_path
        self.crop_h, self.crop_w = crop_size
        self.scale = scale
        ...
        self.files = []
        for name in self.img_ids:
            img_file = osp.join(self.root, "images_jpg/%s.jpg" % name)
            label_file = osp.join(self.root, "masks_png_final/%s.png" %name)
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
...
class PFCNDataTestSet(data.Dataset):
    def __init__(self, root, list_path, crop_size=(128,128), mean=(128, 128, 128)):
        self.root = root
        self.list_path = list_path
        self.crop_h, self.crop_w = crop_size
        self.mean = (104.00698793, 116.66876762, 122.67891434)
